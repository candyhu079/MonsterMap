...
class MapMonsterBattleScene: SKScene {
...
//        if SoundEffect.shareSound().isMute == 1{
//            playSoundEffect=false
//        }
        let userFightingMonsterURL="http://api.leolin.me/myMonsterForFighting"
        let monsterURL="http://api.leolin.me/monster"
        headers=["token":token]
    ...
    changeMonsterNodeBackground=childNodeWithName("changeMonsterBackground") as! SKSpriteNode
        ...
        alamoRequset2(monsterURL) { (inner) -> Void in
            do{
                let result=try inner()
                for i in 0..<result.count{
                    if result[i]["name"] as! String==self.userData?.objectForKey("name") as! String{
                        let monster=SKSpriteNode(color: UIColor(red: 0, green: 0, blue: 0, alpha: 0), size: CGSize(width: 0, height: 0))
                        monster.name="enemyMonsterID1"
                        self.statusEnemyFrameNode.addChild(monster)
                        self.setMonsterUserdata(monster, resultMonster: result, i: i)
                        self.setEnemyMonsterInfo(monster, parentNode: self.statusEnemyFrameNode)
...
//            makeSomeButtonNoise()
            let a=MakeSomeNoise()
            a.makeSomeButtonNoise(buttonVoicePlayer)
    ...
    override func touchesEnded(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if nodeTouchedName == "battleEscapeButton"{
            let runAway:Int=Int(arc4random_uniform(2))
            if runAway == 0{
            reportUserMonsterHP()
            view?.removeFromSuperview()
            }else{
                showSomeWordForOneSec("你想逃但失敗了")
                userMonsterBePause++
...
//        let skillDamageRatio:Float!
        ...
        let runEnemyUseSkill=SKAction.runBlock{ () -> Void in
            ...
            case 1:
                ...
                self.enemySkill1CD=enemyMonsterSkill1CD
                break
            case 2:
                for b in self.monsterSkill2Image{
                    if b.name == (self.statusEnemyFrameNode.childNodeWithName("name") as! SKLabelNode).text{
                ...
                self.enemySkill2CD=enemyMonsterSkill2CD
                break
            case 3:
                for b in self.monsterSkill3Image{
                    if b.name == (self.statusEnemyFrameNode.childNodeWithName("name") as! SKLabelNode).text{
                ...
                self.enemySkill3CD=enemyMonsterSkill3CD
                break
            default:
                break
            }
                ...
                if a.userData?.objectForKey("itemName") as! String == "USB"{
                    ...
                    let catchFailNumber:Int=Int(arc4random_uniform(10))
                        ...
                        if enemyMonsterMaxHP <= 20{
                            ...
                            if enemyMonsterHP < enemyMonsterMaxHP / 5.0{
                                //60%catch
                                if catchFailNumber == 0 || catchFailNumber == 1 || catchFailNumber == 2 || catchFailNumber == 3 {
                                    showSomeWordForOneSec("失敗囉失敗囉失敗囉失敗囉失敗囉")
                                    runAction(SKAction.sequence([SKAction.waitForDuration(1),SKAction.runBlock({ () -> Void in
...
//    func playPressButtonSound(){
//        if playSoundEffect{
//            runAction(SKAction.playSoundFileNamed("button_press.mp3", waitForCompletion: false))
//        }
//    }
}
